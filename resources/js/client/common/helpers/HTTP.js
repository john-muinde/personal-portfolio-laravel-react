...
export const setupInterceptors = (store) => {
    const isPrivate = (config = {}) => {
        return Object.prototype.hasOwnProperty.call(config, "isPrivate") &&
            !config.isPrivate
            ? false
            : true;
    };
        ...
        if (isPrivate(error.config)) {
            let responseStatus =
                ...
                typeof error.response.data.status !== "undefined"
                    ...
                    : CoreConstants.STATUS_CODE_SUCCESS;

            if (responseStatus === CoreConstants.STATUS_CODE_ERROR) {
                if (
                    typeof error.response.data.payload !== "undefined" &&
                    (error.response.data.payload ===
                        CoreConstants.TOKEN_INVALID ||
                        error.response.data.payload ===
                            CoreConstants.TOKEN_BLACKLISTED)
                ) {
                    store.dispatch(removeApiToken());
                }
            } else if (
                responseStatus === CoreConstants.STATUS_CODE_UNAUTHORIZED
            ) {
                if (
                    typeof error.response.data.payload !== "undefined" &&
                    error.response.data.payload === CoreConstants.TOKEN_EXPIRED
                ) {
                    //refresh apiToken
